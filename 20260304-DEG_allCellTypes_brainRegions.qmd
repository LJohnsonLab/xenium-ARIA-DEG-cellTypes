...
# Per-cell-type DE pipeline: pseudobulk DESeq2, Adu vs IgG (all regions pooled)
run_ct_de <- function(obj, ct_label) {
  ...
  # Per-sample heatmap of significant DEGs
  sig_genes <- rownames(res$significant_results)
  if (length(sig_genes) > 0) {
    
    # Extract normalised pseudobulk expression (data layer created by AggregateExpression)
    ...
    # Sample annotation: treatment group
    sample_annot <- data.frame(
      Treatment = bulk$Treatment.Group,
      row.names = colnames(bulk)
    )
    annot_colors <- list(Treatment = c(Adu = "#d73027", IgG = "#4575b4"))
    ...
    pheatmap(expr_scaled,
      ...
      cluster_cols      = TRUE,
      cluster_rows      = ifelse(length(sig_genes) > 1, TRUE, FALSE),
...
A heatmap of log~2~ fold changes (Adu vs IgG) for all genes significant (padj < 0.05) in at
...
if (length(sig_genes_cross) > 0) {
  ...
  display_mat <- ifelse(!is.na(sig_mat) & sig_mat < 0.05, "*", "")
